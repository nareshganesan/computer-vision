...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        # Custom weights initialization as per paper
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                # Conv layers -> uniform distribution
    ...
    def forward(self, x):
        ...
        ## Flatten
        x = x.view(x.size(0), -1) # .view() -> np.reshape
...
class AlexNet(nn.Module):
    ...
    def __init__(self, num_classes: int = 136) -> None:
        ...
        for layer in self.modules():
            if isinstance(layer, nn.Conv2d):
                layer.weight = nn.init.xavier_uniform(layer.weight, gain=1)
